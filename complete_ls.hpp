#ifndef _CompleteLocalSearch_H
#define _CompleteLocalSearch_H

#include <bits/stdc++.h>

using namespace std;

struct CompleteLocalSearch{
    int r; // residents
    int h; // hospitals
    vector<int> max_cap;
    ...
    void solve(int max_itrs){
        ...
        while(iters < max_itrs){
            ...
            if(ubp.size() == 0){
                ...
                int mini = 0;
                int minf = (r+1)*(h+1);
                vector<int> tmp_match_r =curr_match_r;
                vector<priority_queue<pair<int, int>>> tmp_match_h =curr_match_h;
                vector<priority_queue<pair<int, int>>> tmp_match_h_del = curr_match_h_del;
                vector<int> tmp_cap = curr_cap;
                for(int i=0; i<ubp.size(); i++){
                    int re = ubp[i].first;
                    int ho = ubp[i].second;
                    fix_pair(re, ho);
                    int tmpf = cost_of_matching();
                    if(tmpf < minf){
                        mini = i;
                        minf = tmpf;
                    }
                    // reset
                    curr_match_r = tmp_match_r;
                    curr_match_h = tmp_match_h;
                    curr_match_h_del = tmp_match_h_del;
                    curr_cap = tmp_cap;
